...
$(document).ready(function() {
    ...
    $('#addDataRow').off('click').click(async function() {
        ...
        newRow.find('.saveRow').click(async function() { // Wrap this event handler with async
            const row = $(this).closest('tr');
            const performanceId = row.data('performance-id');
            const school_id = $('#schoolIdInput').val();
            const urlParams = new URLSearchParams(window.location.search);
        ...
        if (isDateDuplicate(selectedDate, performanceId, CURRENT_STUDENT_ID, metadata_id)) {
            ...
            try {
                ...
                // Enable the save button after saving
                $(this).prop('disabled', false);
            } catch (error) {
                // Handle any error that occurs during the await
                console.error('Error:', error);
                alert("An error occurred while processing your request.");
                // Enable the save button after an error occurs
                $(this).prop('disabled', false);
            }
        });        
    });
